...
| Item | Value |
|------|-------|
| Static IP | 10.128.0.50 |
| Gateway | 10.128.0.1 |
| DNS Server |  172.30.0.10 |
...
|4 | AAP              |              dev-ansible-automation-platform |stable-2.6 | Required |
...
GODEBUG=http2client=0 oc adm release mirror \
  ...
  --to="registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images" \
  --to-release-image="registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images:4.22.6-x86_64"
```
Expected Output is  the update image,mirror prefix, imageContentSources and ImageContentSourcePolicy
```bash
Success
Update image:registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images:4.22.6-x86_64                              
Mirror prefix: registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images
Mirror prefix: registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images:4.22.6-x86_64
To use the new mirrored repository to install, add the following section to the install-config.yaml
```
```yaml
imageContentSources:
- mirrors:
  - registry-vm-svc.aap-portal-lab.svc.cluster.local:8443/openshift/release-images
...
--workspace file://$(pwd)/oc-mirror-workspace \
  docker://registry-vm-svc.aap-portal-lab.svc.cluster.local:8443 \
...
Cluster resources
- IDMS file  ( filename: idms-oc-mirror.yaml contains Openshift Release and Openshift Catalog. The resource `ImageDigestMirrorSet` mapping source image registries (e.g., `registry.redhat.io`) to your local mirror (`registry-vm-svc.aap-portal-lab.svc.cluster.local:8443`). )
...
#### OSAC Images
Use skopeo copy to push osac images to the mirror registry. Use this script to inspect the pushed images
```bash
#!/bin/bash
MIRROR="registry-vm-svc.aap-portal-lab.svc.cluster.local:8443"
...
for img in "${IMAGES[@]}"; do
  ...
  if digest=$(skopeo inspect --tls-verify=false "${target}" --format '{{.Digest}}' 2>/dev/null); then
    echo " [EXISTS] ${src_path} (Digest: ${digest})"
  else
...
=== Checking Tag-Based Images in Mirror ===
 [EXISTS] osac-project/osac-operator:latest (Digest: sha256:cb73e36d959e74b8739a7598e4f66b3e4ac7e24f3391eb88bf8a380c3a6284cd)
 [EXISTS] osac-project/fulfillment-service:main (Digest: sha256:cece9ee969835c0e6467463cf840b30bf59d7acd4a5447bcbb7864a218f8e76a)
 [EXISTS] osac-project/envoy:v1.33.0 (Digest: sha256:c681cedd8c930397a24f855af1a339d3f0bc4e942a1d4396768bd045fe013f37)
 [EXISTS] osac-project/osac-ui:v0.0.5 (Digest: sha256:87aee616a8eb1ca8c73b4b3b39f2b4ee5b086787e2e84a3b355984f1875013c8)
 [EXISTS] osac-project/osac-aap:latest (Digest: sha256:70914999ca7787d8d11c914399fe4e355326cb5975098e2ebfcfd91d96252266)
 [EXISTS] osac-project/osac-csi-driver:latest (Digest: sha256:96621e1ba03e3d86d8454605d2fdd429cb1612bb850d74ec4a8ec8d2fd10984f)
 [EXISTS] osac-project/metering-service:latest (Digest: sha256:e4183bddc23a240f85bec2d0e945c898753fd12471033d0e55154cfefd3a1acc)
 [EXISTS] osac-project/bare-metal-fulfillment-operator:latest (Digest: sha256:805e312a71f296883c51601fa1976832387d09f994b908e6937c76be6d259491)
 [EXISTS] openbao/openbao:2.6.2 (Digest: sha256:11fd73a2102cda9c55d5d881a8c3210303146a7ec1e8ac76f526e175c6d24641)
 [EXISTS] openshift/origin-cli:4.20.0 (Digest: sha256:ebd858bafa7fe3bf04eda2753d47f74be9608c867f41567cea4af1b1b4189fac)
 [EXISTS] containerdisks/fedora:latest (Digest: sha256:1571d49ee43106e9c7cc4e9f487f6f885ee7c156dc5834fad87ad1e38768b778)

=== Checking Digest-Based Images in Mirror ===
 [EXISTS] sclorg/postgresql-18-c10s@sha256:6be2c9d855f06fb665257a6b0911676a38d740be7022cc61acee1c99a832b1b2 (Digest: sha256:6be2c9d855f06fb665257a6b0911676a38d740be7022cc61acee1c99a832b1b2)
```

# Summary:
For the VMAAS Profile of the OSAC ; Mirrored Registry should have
- OCP 4.22.6
- 6 Operator Catalog essential for OSAC
